"""Tests for providers/opencode.py — HTTP serialisation and error mapping."""
...
def _provider() -> OpenCodeProvider:
    return OpenCodeProvider("test-model", api_key="test-key", max_retries=0)
...
def _opencode_response(content: str = "response text") -> dict:
    return {
        "choices": [
            {
...
def test_complete_returns_content() -> None:
    provider = _provider()
    resp_data = _opencode_response("hello world")
...
def test_retries_on_429_then_succeeds(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    provider = OpenCodeProvider("m", api_key="k", max_retries=3)
    call_count = 0
    ...
    def fake_urlopen(req, timeout=None):
        ...
        cm = MagicMock()
        cm.__enter__ = lambda s: io.BytesIO(json.dumps(_opencode_response("ok")).encode())
        cm.__exit__ = MagicMock(return_value=False)
        return cm
